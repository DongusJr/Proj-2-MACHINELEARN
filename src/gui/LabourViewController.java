...
public class LabourViewController extends VBox implements Initializable
{
  ...
  ObservableList<LabourRow> data = FXCollections.observableArrayList();
   ...
   */
   ...
   public void updateTable(LabourRow row, Profile workerProfile)
   {
     ...
     {
		System.out.println(r.workers.getFirst().profile.equals(workerProfile));
	    System.out.println(r.workers.getFirst().desiredSalary == row.desiredSalary);
		System.out.println(r.workers.getFirst().randomPurchase == row.randomPurchase);
        if(row == r) continue;
