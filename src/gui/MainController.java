...
public class MainController extends Stage implements Initializable
{
  ...
  @FXML private ComboBox<String>     stepSizeBox;
  ...
  private String configFile = "defaultSimulation.json";
  private String exportDir  = "/tmp/SaveSim";
  ...
  public MainController(ModelConfig mConfig, ChartController charts)
  {
    ...
    this.configFile = mConfig.configFile.getText();
    this.exportDir  = mConfig.exportDir.getText();
  ...
  private void initialize()
  {
    ...
    // Set step size box content
    stepSizeBox.getItems().setAll("1 Day", "1 Month", "1 Year", "5 Years", "10 Years");

    stepSizeBox.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
      @Override public void changed(ObservableValue<? extends String> selected, String oldStepSize, String newStepSize) {
        if (newStepSize != null) {
          switch(newStepSize) {
            case "1 Day": stepN = 1; break;
            case "1 Month": stepN = 30; break;
            case "1 Year": stepN = 360; break;
            case "5 Years": stepN = 1800; break;
            case "10 Years": stepN = 3600; break;
          }
        }
      }
    });
  ...
  {
    // As of Java 9, this event is being received twice for each
    // discrete scroll. The only difference between the events is
    // the DeltaY value, so this is being used as a rather
    // dubious way to detect this.
  ...
  /**
   * Export simulation state to this point to CSV. All transactions
   * will be written out from the banks, with accompanying agent
   * information.
   *
   * @param event event from gui
   */
  ...
  {
      File saveDir = new File(exportDir);

      if(! saveDir.exists()) saveDir.mkdirs();

      for (Bank b : simeng.govt.getBankList().values())
      {
          for(Ledger l : b.gl.assets.values())
              l.exportTransactions(exportDir);
          for(Ledger l : b.gl.liabilities.values())
              l.exportTransactions(exportDir);
          for(Ledger l : b.gl.equities.values())
              l.exportTransactions(exportDir);
      }
  }
