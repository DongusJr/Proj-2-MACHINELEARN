...
public class ChartController extends Stage implements Initializable
{
  ...
  public ChartController()
  {
     ...
     */
    flowpane.addEventHandler(MouseEvent.MOUSE_PRESSED,
                    event -> {
                      if (cc == null)
                      {
                        createChartConfig();
  ...
  private void createChartConfig()
  {
    ...
    cc.setOnHidden(event -> {
          for (Node pane : cc.grid.getChildren())
          {
			  if(pane instanceof TitledPane)
			  {
                  for (Node n : ((GridPane)(((TitledPane)pane).getContent())).getChildren() )
				  {
					  if(n instanceof ChartConfig.CBox)
					  {
                         ChartConfig.CBox cb = (ChartConfig.CBox) n;
  ...
  @FXML
  protected void moneyCharts(ActionEvent event)
  {
    disableAllCharts();

    charts.get("money").enabled = true;
    charts.get("newbanklending").enabled = true;
    charts.get("totaldefaults").enabled = true;
    charts.get("reserves").enabled = true;
